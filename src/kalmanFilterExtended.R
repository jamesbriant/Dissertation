...
ApplyExtendedKalmanFilter <- function(Y, f, g, m0, C0, W, V){
  ...
  df.m0 <- jacobian(f, c(m0, 1))
  dg.fm0 <- jacobian(g, c(f.m0, 1))
  ...
  K[[1]] <- CalcKalmanGain(dg.fm0, R[[1]], Q[[1]])
  m[[1]] <- a[[1]] + t(K[[1]]) %*% (Y[[1]] - g.fm0)
  C[[1]] <- (diag(p) -  t(K[[1]]) %*% dg.fm0) %*% R[[1]]
  
  for(t in 2:n){
    f.m <- as.matrix(f(c(m[[t-1]], t)))
    g.fm <- as.matrix(g(c(f.m, t)))
    df.m <- jacobian(f, c(m[[t-1]], t))
    dg.fm <- jacobian(g, c(f.m, t))
    ...
    # update
    K[[t]] <- CalcKalmanGain(dg.fm, R[[t]], Q[[t]])
    m[[t]] <- a[[t]] + t(K[[t]]) %*% (Y[[t]] - g.fm)
    C[[t]] <- (diag(p) - t(K[[t]]) %*% dg.fm) %*% R[[t]]
  }
